 ...
 */
$(function(){
    $('#selArray option:eq(0)').attr('value', `/product/list.html?cate_no=${getParamUrl('cate_no')}&page=1`);
    if (sQueryString && sQueryString.indexOf('sort_method') > -1) {
        for (var i=0; i<$('#selArray option').length; i++) {
            if ($('#selArray option').eq(i).val().indexOf('sort_method='+getParamUrl('sort_method')) > -1) {
